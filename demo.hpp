...
struct DecodeThread : public MyThread {
  ...
  virtual int run() override {
    ...
    //test image resize...it works! RSTP frame was waaaay too big
    cv::Mat image2;
    resize(image, image2, cv::Size(480,360));
    while (!queue_->push(FrameInfo{channel_id_, ++frame_id_, image2},
...
struct GuiThread : public MyThread {
  ...
  virtual int run() override {
    ...
    for (auto &f : frames_) {
      if (f.second.dirty) {
        cv::imshow(std::string{"HASA"}, f.second.frame_info.mat);
        f.second.dirty = false;
        any_dirty = true;
      }
    }
...
struct DpuFilter : public Filter {
  ...
  cv::Mat run(cv::Mat &image) override {
    ...
    //initialize Twilio
    auto twilio_ = std::make_shared<twilio::Twilio>(
  		account_sid, 
        	auth_token); 
    // Twilio inits
    ...
    for (const auto bbox : result.bboxes) {
    	...
    	float ymax = (bbox.y * image.rows) + (bbox.height * image.rows);
    	if(label == 1) { //person
    		//only care about people getting close to the house
    		if(ymax > 300.0) {
    			personCount++;
    			if(personCount == 1) {
    				std::cout << "Person detected\n";
    				message = "Person detected!";
    				message_success = twilio_->send_message(
        		        		to_number, 
        		        		from_number, 
        		        		message,
        		        		response,
    		...
    		//ignore cars in the road, only care about cars in the driveaway
    		if(ymax > 300.0) {
    			carCount++;
    			if(carCount == 1) {
    	...
    	else if(label == 8) {  //truck
    		//ignore trucks in the road, only care about cars in the driveaway
    		if(ymax > 300.0) {
    			truckCount++;
    			if(truckCount == 1) {
    				std::cout << "Truck detected\n";
    				message = "Truck detected in driveway!";
    			message_success = twilio_->send_message(
        		        		to_number, 
        		        		from_number, 
        		        		message,
...
int main_for_video_demo(int argc, char *argv[],
  ...
  {
#if USE_DRM
    //not getting called, not using DRM
    ...
    video_cap.release();
#else
    cv::moveWindow(std::string{"HASA"} + std::to_string(0), 500, 500);
    LOG_IF(INFO, ENV_PARAM(DEBUG_DEMO))
